            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Edit Downloadable</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-md-6">
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Edit Downloadable</h3>
                        </div>
                        <div class="card-body">
                            {{-- Form --}}
                            {!! Form::open(['route' => ['downloadables.update', $data->downloadable_id], 'method' => 'PATCH', 'enctype' => 'multipart/form-data']) !!}
                            <p><span class="required_field">*</span> Required fields.</p>

                            <div class="row">
                                <div class="col-lg-12">
                                    <input type="hidden" name="old_display_name" value="{{$data->display_name}}">
                                    <input type="hidden" name="old_version" value="{{$data->version}}">
                                    <input type="hidden" name="old_downloadable_category_id" value="{{$data->downloadable_category_id}}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="downloadable_category" id="downloadable_category" class="form-control {{$errors->has('downloadable_category') ? 'is-invalid' : ''}}">
                                            ...
                                            @foreach($downloadableCategories as $downloadableCategory)
                                                @if($data->downloadable_category_id == $downloadableCategory->downloadable_category_id)
                                                    <option value="{{$downloadableCategory->downloadable_category_id}}" selected>{{$downloadableCategory->display_name}}</option>
                                                @else
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control{{ $errors->has('version') ? ' is-invalid' : '' }}" name="version" value="{{$data->version}}">
                                    ...
                                    <div class="form-group">
                                        <label for="link">Link <small>(Only fill-up link if using an external link as download link and updating the existing download link)</small></label>
                                    ...
                                    <div class="form-group">
                                        <label for="uploadFile">Upload File <small>(Only add file if updating the existing downloadable file)</small></label>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select multiple class="form-control select2 {{($errors->has('affiliation_access')) ? 'is-invalid' : ''}}" name="affiliation_access[]">
                                            @foreach($affiliations as $affiliation)
                                                @if(in_array($affiliation->affiliation_id, $affiliationAccess))
                                                    <option value="{{$affiliation->affiliation_id}}" selected>{{$affiliation->name}}</option>
                                                @else
